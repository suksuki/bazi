...
export function EnergyFlowChainStrip({ audit, className = "", motionKey }: Props) {
  ...
  return (
    ...
    >
      <p className="mb-1.5 font-medium text-amber-200/90">因果流通链（五行相生）</p>
      ...
      {typeof audit?.abs_threshold === "number" ? (
        <p className="mt-1 text-[9px] text-zinc-600">阈值（归一化场强）: {audit.abs_threshold}</p>
      ) : null}
    </div>
  );
}
