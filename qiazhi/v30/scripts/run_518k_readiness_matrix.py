...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run V30 518K readiness matrix.")
    ...
    if args.json:
        ...
        print(
            "v30.518k_readiness_matrix.v1: "
            f"{'passed' if decision['readiness_matrix_ready'] else 'failed'} "
            f"({decision['passed_check_count']}/{decision['check_count']}) "
            f"{decision['decision_status']}"
        )
    return 0 if decision["readiness_matrix_ready"] else 1
