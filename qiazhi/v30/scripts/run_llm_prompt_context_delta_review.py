...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run V30 CORE-EVIDENCE-3 LLM prompt context delta review.")
    parser.add_argument("--reading-id", default="core-evidence-3-llm-prompt-context")
    parser.add_argument("--json", action="store_true", help="Print full JSON artifact.")
    args = parser.parse_args()

    result = run_llm_prompt_context_delta_review(reading_id=args.reading_id)
    if args.json:
    ...
    print(f"llm_prompt_context_delta_ready={decision['llm_prompt_context_delta_ready']}")
    ...
    return 0 if decision["llm_prompt_context_delta_ready"] else 1
