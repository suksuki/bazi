...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run V30 CORE-EVIDENCE-2 answer quality delta review.")
    parser.add_argument("--reading-id", default="core-evidence-2-answer-quality")
    parser.add_argument("--json", action="store_true", help="Print full JSON artifact.")
    args = parser.parse_args()

    result = run_answer_quality_delta_review(reading_id=args.reading_id)
    if args.json:
    ...
    print(f"answer_quality_delta_ready={decision['answer_quality_delta_ready']}")
    ...
    return 0 if decision["answer_quality_delta_ready"] else 1
