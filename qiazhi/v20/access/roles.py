from __future__ import annotations

from v20.access.schema import AccessRolePolicy


ROLE_POLICIES = {
    "guest": AccessRolePolicy(
        role_key="guest",
        label="游客",
        purpose="Receives an entry-level Bazi reading with simplified portrait and starter questions.",
        allowed_runtime_fields=(
            "version",
            "input_id",
            "locale",
            "bazi_context_frame",
            "context_alignment_report",
            "chart_facts",
            "time_context",
            "core_inference",
            "selected_question",
            "measurement_report",
            "questions",
            "decision_report",
            "structure_dynamics",
            "mainline_arbitration",
            "question_mainline_focus",
            "next_question_plan",
            "brain_state",
            "answer_text",
            "prediction_policy",
            "runtime_mutation",
            "guardrails",
        ),
        blocked_runtime_fields=(
            "feature_layer",
            "chart_graph",
            "rule_paths",
            "knowledge_refs",
            "orchestrator_evidence",
            "brain_memory_signal",
            "orchestrator_policy_observability",
            "llm_capabilities",
            "storage_contract",
            "redis_contract",
        ),
    ),
    "user": AccessRolePolicy(
        role_key="user",
        label="普通用户",
        purpose="Receives bounded Bazi measurement answers and safe topic/question context.",
        allowed_runtime_fields=(
            "version",
            "input_id",
            "locale",
            "bazi_context_frame",
            "context_alignment_report",
            "chart_facts",
            "time_context",
            "core_inference",
            "selected_question",
            "measurement_report",
            "questions",
            "decision_report",
            "feature_state_model",
            "structure_dynamics",
            "mainline_arbitration",
            "question_mainline_focus",
            "next_question_plan",
            "brain_state",
            "orchestrator_policy_pointer",
            "reasoning_orchestrator",
            "question_intent_model",
            "question_agent_state",
            "interaction_session",
            "answer_text",
            "prediction_policy",
            "runtime_mutation",
            "guardrails",
        ),
        blocked_runtime_fields=(
            "chart_graph",
            "rule_paths",
            "knowledge_refs",
            "llm_capabilities",
            "storage_contract",
            "redis_contract",
        ),
    ),
    "analyst": AccessRolePolicy(
        role_key="analyst",
        label="命理师",
        purpose="Reviews current-chart decisions, portrait projection axes, knowledge alignment, LLM assist, and answer boundaries.",
        allowed_runtime_fields=(
            "version",
            "input_id",
            "locale",
            "bazi_context_frame",
            "context_alignment_report",
            "chart_facts",
            "time_context",
            "core_inference",
            "feature_layer",
            "knowledge_report",
            "knowledge_refs",
            "knowledge_alignment",
            "knowledge_semantic_model",
            "knowledge_semantic_validation",
            "decision_report",
            "decision_validation",
            "feature_state_model",
            "structure_dynamics",
            "mainline_arbitration",
            "question_mainline_focus",
            "next_question_plan",
            "brain_state",
            "brain_memory_signal",
            "orchestrator_policy_pointer",
            "orchestrator_policy_observability",
            "question_source_ranking_report",
            "reasoning_orchestrator",
            "orchestrator_evidence",
            "question_intent_model",
            "question_agent_state",
            "interaction_session",
            "questions",
            "selected_question",
            "practitioner_session",
            "measurement_report",
            "answer_plan",
            "answer_text",
            "prediction_policy",
            "llm_assist",
            "runtime_mutation",
            "guardrails",
        ),
        blocked_runtime_fields=("llm_capabilities", "raw_private_feedback", "secret_config"),
    ),
    "lab": AccessRolePolicy(
        role_key="lab",
        label="实验室",
        purpose="Runs dry-run corpus, validation, and learning experiments without untraced runtime changes.",
        allowed_runtime_fields=(
            "version",
            "input_id",
            "locale",
            "bazi_context_frame",
            "context_alignment_report",
            "chart_facts",
            "time_context",
            "core_inference",
            "chart_graph",
            "rule_paths",
            "feature_layer",
            "knowledge_report",
            "knowledge_refs",
            "knowledge_alignment",
            "knowledge_semantic_model",
            "knowledge_semantic_validation",
            "decision_report",
            "decision_validation",
            "feature_state_model",
            "structure_dynamics",
            "mainline_arbitration",
            "question_mainline_focus",
            "next_question_plan",
            "brain_state",
            "brain_memory_signal",
            "orchestrator_policy_pointer",
            "orchestrator_policy_observability",
            "question_source_ranking_report",
            "reasoning_orchestrator",
            "orchestrator_evidence",
            "question_intent_model",
            "question_agent_state",
            "interaction_session",
            "questions",
            "selected_question",
            "practitioner_session",
            "measurement_report",
            "answer_plan",
            "prediction_policy",
            "llm_capabilities",
            "llm_assist",
            "runtime_mutation",
            "guardrails",
        ),
        blocked_runtime_fields=("secret_config", "untraced_runtime_override_write"),
    ),
    "admin": AccessRolePolicy(
        role_key="admin",
        label="管理员",
        purpose="Views admin-only observability for portrait projection, latent calibration, rule hits, and runtime guardrails; secrets remain hidden.",
        allowed_runtime_fields=(
            "version",
            "input_id",
            "locale",
            "bazi_context_frame",
            "context_alignment_report",
            "chart_facts",
            "time_context",
            "core_inference",
            "feature_layer",
            "knowledge_report",
            "knowledge_refs",
            "knowledge_alignment",
            "knowledge_semantic_model",
            "knowledge_semantic_validation",
            "decision_report",
            "decision_validation",
            "feature_state_model",
            "structure_dynamics",
            "mainline_arbitration",
            "question_mainline_focus",
            "next_question_plan",
            "brain_state",
            "brain_memory_signal",
            "orchestrator_policy_pointer",
            "orchestrator_policy_observability",
            "question_source_ranking_report",
            "reasoning_orchestrator",
            "orchestrator_evidence",
            "question_intent_model",
            "question_agent_state",
            "interaction_session",
            "questions",
            "selected_question",
            "portrait_graph_summary",
            "practitioner_session",
            "latent_signal_report",
            "latent_event_session",
            "measurement_report",
            "answer_plan",
            "answer_text",
            "prediction_policy",
            "llm_assist",
            "runtime_mutation",
            "guardrails",
        ),
        blocked_runtime_fields=("secret_config", "raw_private_feedback", "redis_ephemeral_values"),
    ),
}


def role_policy(role_key: str) -> AccessRolePolicy:
    try:
        return ROLE_POLICIES[role_key]
    except KeyError as exc:
        raise ValueError(f"Unknown V20 access role: {role_key}") from exc


def access_role_manifest() -> dict[str, object]:
    return {
        "version": "v20.access_role_manifest.v1",
        "roles": [policy.to_dict() for policy in ROLE_POLICIES.values()],
        "runtime_mutation": False,
        "guardrails": [
            "ACCESS_CONTRACT_ONLY",
            "AUTHENTICATION_LAYER_CAN_WRAP_THIS_POLICY",
            "NO_SECRET_VALUES_RENDERED",
        ],
    }
