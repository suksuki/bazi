...
DEFAULT_RUNTIME_DIR="${V20_RUNTIME_DIR:-v20/.runtime/local}"
...
start_service() {
  ...
  if command -v screen >/dev/null 2>&1; then
    ...
    screen -dmS "${SCREEN_NAME}" bash -lc "cd '${ROOT_DIR}' && exec '${SCRIPT_DIR}/start_macos.sh' >> '${LOG_FILE}' 2>&1"
  else
...
stop_service() {
  ...
  if ! pid="$(running_pid)"; then
    rm -f "${PID_FILE}"
    if pid="$(port_pid)" && [[ -n "${pid}" ]]; then
      echo "Stopping unmanaged V20 macOS listener on port ${V20_PORT} pid=${pid}"
      ...
      echo "Stopped unmanaged V20 macOS listener"
      screen -S "${SCREEN_NAME}" -X quit >/dev/null 2>&1 || true
      return 0
    fi
    screen -S "${SCREEN_NAME}" -X quit >/dev/null 2>&1 || true
    echo "V20 macOS service is not running"
    return 0
  fi
