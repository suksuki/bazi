...
from v20.learning.role_interaction_training import build_role_interaction_training_report  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build the V20 role interaction candidate training report.")
    ...
    def _run() -> dict[str, object]:
        if args.progress:
            print("[v20-role-interaction] building candidate policy", file=sys.stderr, flush=True)
    ...
    return run_and_print(
        _run,
        command="run_role_interaction_training.py",
        args=args,
...
def _write_report(report: dict[str, object]) -> dict[str, object]:
    directory = local_jsonl_store_from_env().runtime_dir / "training" / "role_interaction"
    directory.mkdir(parents=True, exist_ok=True)
    latest_path = directory / "latest.json"
    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_path = directory / f"role_interaction_training_{stamp}.json"
    payload = report | {"written_at": datetime.now(timezone.utc).isoformat(), "runtime_mutation": True}
    text = json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True)
    latest_path.write_text(text, encoding="utf-8")
    run_path.write_text(text, encoding="utf-8")
    return {
        "version": "v20.role_interaction_training_artifact_write.v1",
        "status": "written",
        ...
        "guardrails": [
            ...
            "ROLE_INTERACTION_POLICY_REMAINS_CANDIDATE",
        ],
    }
