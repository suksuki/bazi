...
from v20.learning.rule_replay_eval import (  # noqa: E402
    build_rule_replay_eval_report,
    read_rule_replay_eval_artifact,
    write_rule_replay_eval_artifact,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Build V20 offline rule replay eval evidence.")
    ...
    if args.status:
        payload = read_rule_replay_eval_artifact()
    elif args.write:
        payload = write_rule_replay_eval_artifact(
            domain=args.domain,
        ...
        payload = build_rule_replay_eval_report(
            args.domain,
            limit=max(1, args.limit),
            per_rule=max(1, args.per_rule),
            progress=progress,
