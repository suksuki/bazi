...
MODULES: tuple[ModuleMigrationStatus, ...] = (
    ...
    ModuleMigrationStatus(
        key="knowledge_cards",
        label="知识卡与解释依据",
        current_state="v40_native_v1_explanation_sidecar_ready",
        reuse_policy="migrate_as_knowledge_card_not_judge",
        v30_sources=("knowledge", "docs/bazi_knowledge"),
        v40_target="enrichment/sidecar",
        rc2_action="V1 已从运行时信号生成 explanation-only knowledge card signal，不进入 verdict 输入。",
    ),
    ModuleMigrationStatus(
        key="portrait_signals",
        label="画像信号",
        current_state="v40_native_v1_low_weight_signal_ready",
        reuse_policy="migrate_as_low_weight_portrait_signal",
        v30_sources=("portrait", "diagnosis/portrait_engine"),
        v40_target="enrichment/sidecar",
        rc2_action="V1 已生成低权重 portrait RuntimeSignal，可进入候选排序但不能强断。",
    ),
    ModuleMigrationStatus(
    ...
    ModuleMigrationStatus(
        ...
        current_state="v40_sidecar_v1_enriched",
        reuse_policy="sidecar_asset_migration_only",
        v30_sources=("ziwei",),
        v40_target="engines/ziwei_native + enrichment/sidecar",
        rc2_action="紫微继续保持 zero decision weight，并新增 sidecar enrichment 供命理师 Lens 和追问参考。",
    ),
    ModuleMigrationStatus(
