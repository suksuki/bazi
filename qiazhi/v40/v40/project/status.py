...
CURRENT_PHASE = 47
CURRENT_PHASE_NAME = "Probe Answer Runtime"
...
DOMAINS: tuple[CompletionDomain, ...] = (
    ...
    CompletionDomain(
        ...
        next_step="把 ProbeAnswerResult 接入后续对话上下文，替换 URL role hook，并做桌面/移动浏览器视觉验收。",
    ),
    CompletionDomain(
...
PHASE_GROUPS: tuple[dict[str, object], ...] = (
    ...
    {"range": "47", "label": "Probe answer runtime", "status": "active"},
    {"range": "48+", "label": "conversation uses probe answers、auth role、visual QA、user acceptance、online cutover", "status": "requires_user"},
)
...
def build_project_status(*, lab_summary: dict[str, Any] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "next_mainline_tasks": [
            "UI-5: feed ProbeAnswerResult into conversation context",
            "UI-6: replace URL role hook with auth-derived role context",
