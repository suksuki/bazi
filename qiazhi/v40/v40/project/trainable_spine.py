...
def build_trainable_runtime_spine_status() -> dict[str, object]:
    return {
        ...
        "feedback_flow": [
            ...
            "BatchTrainerV1 creates candidate policy versions",
            "TrainingImpactDiff explains changed weights, thresholds, verdicts, advice and probes",
            "ReleaseGate approves, rejects or rolls back candidate policy",
        ],
        "hard_gates": [
            "Do not train chart facts",
            "Do not let one user feedback mutate global policy",
            "Do not let LLM become the mingli training target",
            "Every global update must be replayable, explainable and rollbackable",
            "Every candidate policy must pass golden, regression, overclaim, advice grounding, probe yield, leakage and LLM boundary gates",
        ],
        "contracts": [
            "TrainableUnit",
        ...
        "next_steps": [
            "Persist TrainablePolicyRegistry versions",
            "Implement BatchTrainerV1 for source_weight/rule_weight/advice_priority/probe_voi/assertion_threshold",
            "Add before/after candidate policy diff to Acceptance Window",
            "Expose policy version used in every runtime record",
        ],
