...
def run_native_bazi_engine(*, engine_request: EngineRunRequest, chart: BaziChartFacts) -> EngineRunResult:
    ...
    fact_engine_pro_profile = build_fact_engine_pro_profile(chart)
    facts = [
        ...
        {"fact_id": "adapter.fact_engine_pro", "label": "事实层 Pro", "value": fact_engine_pro_profile},
        {
            "fact_id": "adapter.hidden_stems",
            "label": "地支藏干",
            "value": fact_engine_pro_profile["hidden_stems"],
        },
    ]
    features = [
        ...
        {
            "feature_id": "feature.hidden_ten_god_counts",
            "label": "藏干十神计数",
            "value": fact_engine_pro_profile["hidden_ten_god_counts"],
        },
        {
            "feature_id": "feature.root_profile",
            "label": "日主根气",
            "value": fact_engine_pro_profile["root_profile"],
        },
        {
            "feature_id": "feature.month_authority",
            "label": "月令权重",
            "value": fact_engine_pro_profile["month_profile"],
        },
        {
            "feature_id": "feature.advanced_branch_relation_counts",
            "label": "复杂地支关系",
            "value": fact_engine_pro_profile["advanced_branch_relations"]["counts"],
        },
    ]
    signals = _build_signals(
        ...
        fact_engine_pro_profile=fact_engine_pro_profile,
...
def _build_signals(
    *,
    ...
    fact_engine_pro_profile: dict[str, object],
    support_score: float,
    ...
    signals = [
        ...
        RuntimeSignal(
            signal_id=f"{reading_id}:fact-pro",
            reading_id=reading_id,
            source=SignalSource.BAZI_ENGINE,
            source_ref="native_bazi_fact_engine_pro",
            topic=Topic.STRUCTURE,
            claim=_fact_pro_claim(fact_engine_pro_profile),
            polarity=Polarity.NEUTRAL,
            strength=_fact_pro_strength(fact_engine_pro_profile),
            confidence=0.68,
            assertion_hint=AssertionLevel.SUPPORTED,
            evidence_refs=fact_pro_evidence,
            trainable_targets=["signal_weight.bazi_fact_pro_root_season"],
        ),
        RuntimeSignal(
        ...
        RuntimeSignal(
            signal_id=f"{reading_id}:hidden-ten-god",
            reading_id=reading_id,
            source=SignalSource.BAZI_ENGINE,
            source_ref="native_bazi_fact_engine_pro",
            topic=Topic.STRUCTURE,
            claim=_hidden_ten_god_claim(fact_engine_pro_profile),
            polarity=Polarity.NEUTRAL,
            strength=_hidden_ten_god_strength(fact_engine_pro_profile),
            confidence=0.66,
            assertion_hint=AssertionLevel.SUPPORTED,
            evidence_refs=[*fact_pro_evidence, "feature.hidden_ten_god_counts"],
            trainable_targets=["signal_weight.hidden_ten_god"],
        ),
        RuntimeSignal(
        ...
        RuntimeSignal(
            signal_id=f"{reading_id}:advanced-branch-relations",
            reading_id=reading_id,
            source=SignalSource.BAZI_ENGINE,
            source_ref="native_bazi_fact_engine_pro",
            topic=Topic.TIMING,
            claim=_advanced_branch_relation_claim(fact_engine_pro_profile),
            polarity=_advanced_branch_relation_polarity(fact_engine_pro_profile),
            strength=_advanced_branch_relation_strength(fact_engine_pro_profile),
            confidence=0.64,
            assertion_hint=AssertionLevel.MIXED,
            evidence_refs=[*branch_evidence, "feature.advanced_branch_relation_counts"],
            trainable_targets=["signal_weight.advanced_branch_relation"],
        ),
        RuntimeSignal(
...
def _domain_signals(
    *,
    ...
    if not isinstance(counts, dict):
        counts = {}
    hidden_counts = fact_engine_pro_profile.get("hidden_ten_god_counts") or {}
    if not isinstance(hidden_counts, dict):
        hidden_counts = {}
    wealth_count = float(ten_god_profile.get("wealth_count") or 0) + _hidden_family_count(
        hidden_counts, ["正财", "偏财"]
    )
    officer_count = float(ten_god_profile.get("officer_count") or 0) + _hidden_family_count(
        hidden_counts, ["正官", "七杀"]
    )
    output_count = float(ten_god_profile.get("output_count") or 0) + _hidden_family_count(
        hidden_counts, ["食神", "伤官"]
    )
    relation_count = int(branch_relation_profile.get("clash_count") or 0) + int(
        branch_relation_profile.get("harmony_count") or 0
    )
    advanced_relation_count = int(
        (fact_engine_pro_profile.get("advanced_branch_relations") or {}).get("relation_count") or 0
    )
    relation_total = relation_count + advanced_relation_count
    ...
    signals = [
        RuntimeSignal(
            ...
            evidence_refs=[*evidence, "adapter.ten_god_profile", "feature.useful_god", "adapter.fact_engine_pro"],
        ...
        RuntimeSignal(
            ...
            polarity=Polarity.MIXED if relation_total else Polarity.NEUTRAL,
            strength=round(min(0.78, 0.42 + officer_count * 0.1 + wealth_count * 0.08 + relation_total * 0.04), 2),
            confidence=0.56,
            assertion_hint=AssertionLevel.MIXED,
            evidence_refs=[*evidence, "adapter.ten_god_profile", "adapter.branch_relations", "adapter.fact_engine_pro"],
        ...
        RuntimeSignal(
            ...
            polarity=Polarity.MIXED if relation_total else Polarity.NEUTRAL,
            strength=round(min(0.76, 0.4 + relation_total * 0.06), 2),
            confidence=0.52 if relation_total else 0.48,
            assertion_hint=AssertionLevel.WEAK_CANDIDATE,
            evidence_refs=[*evidence, "adapter.branch_relations", "adapter.fact_engine_pro"],
...
def _fact_pro_claim(profile: dict[str, object]) -> str:
    root = profile.get("root_profile") or {}
    month = profile.get("month_profile") or {}
    if not isinstance(root, dict):
        root = {}
    if not isinstance(month, dict):
        month = {}
    root_level = str(root.get("root_level") or "根气待复核")
    month_branch = str(month.get("month_branch") or "月令")
    relation = _month_relation_label(str(month.get("relation_to_day") or "unknown"))
    branches = root.get("root_branches") or []
    branch_text = "、".join(str(row) for row in branches) if isinstance(branches, list) and branches else "未显"
    return f"事实层复核显示日主{root_level}，根气落点在{branch_text}；月令{month_branch}对日主呈{relation}，用神和强弱需先按根气与月令校准。"


def _fact_pro_strength(profile: dict[str, object]) -> float:
    root = profile.get("root_profile") or {}
    month = profile.get("month_profile") or {}
    if not isinstance(root, dict):
        root = {}
    ...
    month_score = float(month.get("authority_score") or 0.5)
    return round(max(0.44, min(0.82, (root_score + month_score) / 2)), 2)


def _hidden_ten_god_claim(profile: dict[str, object]) -> str:
    counts = profile.get("hidden_ten_god_counts") or {}
    if not isinstance(counts, dict) or not counts:
        return "藏干十神暂未形成有效计数，领域判断先用显性天干和地支关系。"
    ranked = sorted(counts.items(), key=lambda item: (-float(item[1]), str(item[0])))[:3]
    text = "、".join(f"{name}{round(float(score), 1)}" for name, score in ranked)
    return f"藏干十神以{text}较显，说明暗处仍有可用证据，不能只按天干显性十神下结论。"


def _hidden_ten_god_strength(profile: dict[str, object]) -> float:
    counts = profile.get("hidden_ten_god_counts") or {}
    if not isinstance(counts, dict) or not counts:
        return 0.42
    return round(min(0.78, 0.42 + max(float(value) for value in counts.values()) * 0.12), 2)
...
def _advanced_branch_relation_claim(profile: dict[str, object]) -> str:
    advanced = profile.get("advanced_branch_relations") or {}
    if not isinstance(advanced, dict):
        advanced = {}
    labels = _relation_labels(advanced.get("relations") or [])
    if labels:
        return f"复杂地支关系补充看到{'、'.join(labels[:3])}，适合作为隐藏互动、阶段触发和现实反馈校准点。"
    return "复杂地支关系未形成明显刑害破或三合三会入口，先不制造额外动态结论。"


def _advanced_branch_relation_polarity(profile: dict[str, object]) -> Polarity:
    advanced = profile.get("advanced_branch_relations") or {}
    if not isinstance(advanced, dict):
        return Polarity.NEUTRAL
    counts = advanced.get("counts") or {}
    if not isinstance(counts, dict):
        return Polarity.NEUTRAL
    if any(int(counts.get(key) or 0) for key in ["harm", "break", "punishment"]):
        return Polarity.MIXED
    if any(int(counts.get(key) or 0) for key in ["three_harmony", "three_meeting"]):
        return Polarity.SUPPORT
    return Polarity.NEUTRAL


def _advanced_branch_relation_strength(profile: dict[str, object]) -> float:
    advanced = profile.get("advanced_branch_relations") or {}
    if not isinstance(advanced, dict):
        return 0.42
    count = int(advanced.get("relation_count") or 0)
    return round(min(0.78, 0.42 + count * 0.05), 2)
...
def _hidden_family_count(counts: dict[object, object], names: list[str]) -> float:
    return round(sum(float(counts.get(name) or 0.0) for name in names), 3)


def _wealth_claim(*, wealth_count: float, output_count: float, candidate_text: str) -> str:
    if wealth_count:
...
def _relationship_claim(*, chart: BaziChartFacts, wealth_count: float, officer_count: float) -> str:
...
def _health_claim(
    *,
    branch_relation_profile: dict[str, object],
    fact_engine_pro_profile: dict[str, object] | None = None,
) -> str:
    advanced = {}
    if isinstance(fact_engine_pro_profile, dict):
        advanced = fact_engine_pro_profile.get("advanced_branch_relations") or {}
    if not isinstance(advanced, dict):
        advanced = {}
...
def _month_relation_label(relation: str) -> str:
    return {
        "same": "同气扶身",
        "resource": "资源生扶",
        "output": "输出泄身",
        "wealth": "财星牵引",
        "officer_pressure": "官杀压力",
        "neutral": "中性",
        "unknown": "待复核",
    }.get(relation, "待复核")
