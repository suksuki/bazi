...
def _run_v30(*, fixture: dict[str, Any], reading_id: str) -> CognitiveBenchmarkReading:
    v30_root = ROOT.parent / "v30"
    if str(v30_root) not in sys.path:
        sys.path.insert(0, str(v30_root))
    from v30.core.chart_context import build_chart_context_from_displays  # noqa: PLC0415
    from v30.presentation import build_presentation_model  # noqa: PLC0415
    from v30.runtime import create_runtime_from_context  # noqa: PLC0415
    ...
    synthesis = _mapping(surface.get("final_synthesis"))
    structure = _mapping(surface.get("structure_dynamics"))
    summary = _mapping(surface.get("reading_summary"))
    verdicts = [row for row in synthesis.get("decision_verdicts", []) if isinstance(row, dict)]
    primary = verdicts[0] if verdicts else {}
    alternative = verdicts[1] if len(verdicts) > 1 else {}
    paths = [row for row in structure.get("top_paths", []) if isinstance(row, dict)]
    main_path = _path_text(paths[0]) if paths else str(summary.get("diagnosis_overview") or "")
    secondary_paths = [_path_text(row) for row in paths[1:4] if _path_text(row)]
    evidence_refs = _unique(
        [
            ref
            for row in synthesis.get("evidence_chain", [])
            if isinstance(row, dict)
            for ref in row.get("evidence", [])
        ]
    )
    career_card = _domain_card(surface, "career")
    wealth_card = _domain_card(surface, "wealth")
    uncertainty = [
        "V30 runtime is mechanically projected into the common contract; absent fields are not LLM-filled.",
        *[str(item) for item in getattr(runtime.answer_context, "forbidden_drift", [])],
    ]
    return CognitiveBenchmarkReading(
        independent_first_look=str(summary.get("diagnosis_overview") or summary.get("primary_message") or ""),
        chart_center_of_gravity=str(synthesis.get("conclusion") or summary.get("final_conclusion") or ""),
        primary_hypothesis=BenchmarkHypothesis(
            name=str(primary.get("headline") or "V30 primary verdict"),
            thesis="；".join(str(item) for item in primary.get("allowed_assertions", [])),
            confidence=_confidence(primary.get("confidence")),
            supporting_evidence_refs=_unique(primary.get("supporting_evidence", []) or evidence_refs[:4]),
            failure_conditions=[str(synthesis.get("risk_boundary") or "")],
        ),
        strongest_alternative=BenchmarkHypothesis(
            name=str(alternative.get("headline") or "V30 未形成整盘竞争假设"),
            thesis="；".join(str(item) for item in alternative.get("allowed_assertions", [])),
            confidence=_confidence(alternative.get("confidence")),
        ),
        why_primary_over_alternative=[
            str(synthesis.get("decision_focus") or "V30 does not expose a whole-chart comparative reason."),
        ],
        main_work_path=main_path,
        secondary_or_blocked_paths=secondary_paths,
        critical_nodes=_unique([str(item) for row in paths[:4] for item in row.get("nodes", [])]),
        pivot_candidates=[],
        bridge_or_support_candidates=[],
        body_function_relation="V30 runtime does not expose a unified body-function comparison contract.",
        conditional_useful_roles=[],
        conditional_harmful_roles=[],
        stable_portrait=[str(summary.get("primary_message") or "")],
        hidden_attribute_candidates=[],
        career_reasoning=_v30_domain(career_card),
        wealth_reasoning=_v30_domain(wealth_card),
        prior_predictions=[],
        falsifiers=[str(synthesis.get("risk_boundary") or "")],
        discriminating_probe=BenchmarkProbe(
            question=str(surface.get("next_question") or "V30 未形成统一鉴别问题"),
            purpose="V30 projected next question",
        ),
        known_uncertainties=_unique(uncertainty),
...
def _v30_domain(card: dict[str, Any]) -> BenchmarkDomainReasoning:
    summary = str(card.get("diagnosis_summary") or card.get("summary") or "")
    claims = [row for row in card.get("diagnosis_claims", []) if isinstance(row, dict)]
    return BenchmarkDomainReasoning(
        causal_chain=[summary] if summary else [],
        stable_tendencies=_unique([str(row.get("claim") or row.get("text") or "") for row in claims]),
        unknowns=["V30 domain output does not expose the full common causal contract."],
        evidence_refs=_unique([str(ref) for row in claims for ref in row.get("evidence_ids", [])]),
    )


def _domain_card(surface: dict[str, Any], domain: str) -> dict[str, Any]:
    for row in surface.get("domain_cards", []):
...
def _path_text(row: dict[str, Any]) -> str:
    return str(row.get("diagnosis_statement") or row.get("summary") or row.get("path_statement") or "")
...
def _confidence(value: Any) -> str:
    try:
        ...
        return "unresolved"
    return "high" if number >= 0.75 else "medium" if number >= 0.5 else "low"
