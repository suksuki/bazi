...
BAZI_SYMBOLIC_KNOWLEDGE_ID = "v17.knowledge.bazi_symbolic_primitives.v1"
BAZI_SYMBOLIC_KNOWLEDGE_PATH = V17_REBIRTH_ROOT / "backend" / "logic" / "knowledge" / "bazi_symbolic_primitives.v1.json"
...
@lru_cache(maxsize=1)
def load_bazi_symbolic_knowledge() -> Dict[str, Any]:
    return json.loads(BAZI_SYMBOLIC_KNOWLEDGE_PATH.read_text(encoding="utf-8"))


def _symbolic_knowledge_section(name: str) -> Dict[str, Any]:
    section = load_bazi_symbolic_knowledge().get(name)
    return dict(section) if isinstance(section, Mapping) else {}


PALACE_CONTEXT: Dict[str, Dict[str, str]] = {
    str(key): {str(item_key): str(item_value) for item_key, item_value in value.items()}
    for key, value in _symbolic_knowledge_section("palace_context").items()
    if isinstance(value, Mapping)
}

STEM_IMAGE_RULES: Dict[str, Dict[str, Any]] = {
    str(key): dict(value)
    for key, value in _symbolic_knowledge_section("stem_image_rules").items()
    if isinstance(value, Mapping)
}

BRANCH_IMAGE_RULES: Dict[str, Dict[str, Any]] = {
    str(key): dict(value)
    for key, value in _symbolic_knowledge_section("branch_image_rules").items()
    if isinstance(value, Mapping)
}

VAULT_RULES: Dict[str, Dict[str, str]] = {
    str(key): {str(item_key): str(item_value) for item_key, item_value in value.items()}
    for key, value in _symbolic_knowledge_section("vault_rules").items()
    if isinstance(value, Mapping)
}

TEN_GOD_FAMILY: Dict[str, Dict[str, str]] = {
    str(key): {str(item_key): str(item_value) for item_key, item_value in value.items()}
    for key, value in _symbolic_knowledge_section("ten_god_family").items()
    if isinstance(value, Mapping)
}

RELATION_TAGS: Dict[str, str] = {
    str(key): str(value)
    for key, value in _symbolic_knowledge_section("relation_tags").items()
}
...
def normalize_bazi_image_meta(value: Any) -> Dict[str, Any]:
    ...
    normalized["knowledge_base"] = dict(knowledge) if isinstance(knowledge, Mapping) else {
        "id": BAZI_SYMBOLIC_KNOWLEDGE_ID,
        "mode": "versioned_symbolic_knowledge",
    }
    return normalized
...
def resolve_bazi_image(physics_tensor: Dict[str, Any]) -> Dict[str, Any]:
    ...
    image = {
        ...
        "knowledge_base": {
            "id": BAZI_SYMBOLIC_KNOWLEDGE_ID,
            "mode": _clean_label(load_bazi_symbolic_knowledge().get("mode")) or "versioned_symbolic_knowledge",
            "version": _clean_label(load_bazi_symbolic_knowledge().get("version")),
        },
