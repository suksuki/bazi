...
def _weaver_system_core(*, will_proxy: str, decision_anchor: str, action_signal: bool) -> str:
    ...
    blocks = [
        ...
        "【十神物理解释】\n若输入出现 ten_gods_base_l0 / ten_gods_runtime，请将其理解为绝对物理强度，而非百分比。单个十神分值并非单一来源，而是显化、根气、势能、潜藏残值的合成。根气偏“扎根”，势能偏“得势”；例如丙见巳偏根强，丙见午偏势强。若输入出现做功方向矩阵 / 做功回路，前者表示 source 对 target 的净推动或净压制，后者表示双向关系是同向放大还是对冲拉扯。若输入出现合化摘要 / 格局摘要，其中百分比表示成局度或拟合度，不是十神能量百分比；须与“基准x”及绝对强度合读。",
...
def _judge_system_core(*, will_proxy: str, decision_anchor: str, action_signal: bool) -> str:
    ...
    blocks = [
        ...
        "【十神物理解释】\n若输入出现 ten_gods_base_l0 / ten_gods_runtime，请将其理解为绝对物理强度，而非百分比。单个十神分值并非单一来源，而是显化、根气、势能、潜藏残值的合成。根气回答“是否扎根”，势能回答“是否得势”，两者不可混为一谈。若输入出现做功方向矩阵 / 做功回路，前者表示 source 对 target 的净推动或净压制，后者表示双向关系是同向放大还是对冲拉扯。若输入出现合化摘要 / 格局摘要，其中百分比表示成局度或拟合度，不是十神能量百分比；须与“基准x”及绝对强度合读。",
        "【显著性】\n以下提供的 160 条事实已按显著性（Salience）降序排列。排序越靠前的事实对命局的影响越具决定性。请务必优先回应前 10 条核心事实，将其作为你裁决的第一物理支点。",
