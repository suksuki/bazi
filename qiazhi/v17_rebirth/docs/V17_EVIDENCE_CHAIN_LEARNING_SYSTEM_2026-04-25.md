...
命理师也可以在排盘页“体用中枢”原位查看系统返回的格局、用神、忌神候选及置信度，并选择本次断语采用项。该选择进入 `v17.practitioner.override_context.v1`，只影响本次叙事生成，不直接修改参数、候选排序或学习治理状态。
...
- 证据链 UI 已增加证据目录筛选：全部、强证据、候选、反例、边界、需复核；展开数量按当前筛选结果计算。
...
1. 让 LLM Analyst 只基于这些结构化素材生成学习候选解释，不直接改 evidence 或参数。
2. 将 shadow-run 报告生成器升级为安全 A/B Experiment Runner，比较 baseline 与候选 patch 后再允许 `promote`。
