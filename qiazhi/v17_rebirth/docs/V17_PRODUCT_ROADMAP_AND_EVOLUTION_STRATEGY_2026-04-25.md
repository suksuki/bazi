...
- `/v17/auth/practitioner-learning-shadow-run` 可基于实验队列生成只读 shadow-run 报告：运行当前 synthetic baseline，审计静态与 accepted practitioner benchmarks，并输出可直接导入 scorecard 的报告结构；候选 patch 仍不自动应用。
...
- 物理快照新增 `v17.practitioner.choice_candidates.v1`，由系统返回格局、用神、忌神候选及置信度；命理师可在排盘页“体用中枢”原位选择本次断语采用项。
...
- 财富专属 Prompt 合同已新增 `v17.topic.wealth_assertion_prompt.v1.0`：只消费 `wealth_profile`，规定财富总断、财富来源、可用与承接、风险、行动五段；当前为可调用 builder，尚未接 UI 或真实 LLM 调用。
...
2. 将 shadow-run 报告生成器升级为真正的 A/B Experiment Runner：在安全沙盒中比较 baseline 与候选 patch，而不是只跑当前基线。
