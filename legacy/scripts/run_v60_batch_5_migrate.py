#!/usr/bin/env python3
"""
FDS SOP V6.0：第五梯队（A-41～A-50）Phase 3 迁库
==================================================
A-41（壬骑龙背）与 A-43（拱禄格）全量迁入，不设 limit；其余按 --limit 执行。
"""
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="V6.0 第五梯队 Phase 3 迁库（A-41/A-43 全量）")
    p.add_argument("--data", type=Path, default=None)
    p.add_argument("--abundance", type=Path, default=None, help="默认 audit_logs/v60_batch_5_abundance.json")
    p.add_argument("--limit", type=int, default=1500, help="每格局最多迁入条数（默认 1500）；A-41/A-43 不受此限")
    args = p.parse_args()
    ...
    abundance_path = args.abundance or (ROOT / "audit_logs" / "v60_batch_5_abundance.json")
    if not abundance_path.exists():
        print(f"❌ 未找到丰度报告: {abundance_path}，请先运行 run_v60_batch_5_scan.py")
        sys.exit(1)
        ...
        for i, line in enumerate(f):
            ...
            for pid in matched:
                if pid not in by_pattern:
                    continue
                bag = by_pattern[pid]
                cap = None if pid in ("A-41", "A-43") else args.limit
