...
| （V4.x 历史稿） | — | 已自仓库移除；执行以 V5.x / V5.5 为准 |
...
**Step 6 物理生产（深水区海选）**：① A-11 日主坍缩度审计 `legacy/scripts/audit/audit_a11_e_collapse.py`；② A-13 流形纯度审计 `legacy/scripts/audit/audit_a13_purity.py`；③ A-11/A-12/A-13 518k 海选并迁入 DuckDB `legacy/scripts/run_a11_a12_a13_scan_and_migrate.py`；④ 法理一致性终审 `legacy/scripts/audit/v5_5_extreme_patterns_alignment.py` → `legacy/audit_logs/v5_5_extreme_patterns_alignment.json`；⑤ A-12 RAG 判词抽检验收要点见 `audit/A12_RAG_VERDICT_CHECKLIST.md`。**审计师封卷导引**见 `audit/FDS_046_封卷导引与物理奇点.md`。**第 046 号已进入法典封存状态。**

**后续建议（全量对撞测试）**：命例同时命中多格局时的判词验收，首轮案例 **A-07（伤官）∩ A-12（从杀）**（含「法理绞杀」下极致对冲建议）；A-13「能量孤岛」流形观测；验收标准与执行方式见上述封卷导引第三至六节。

**第 049/050 号 动态时空位移引擎（SOP V5.6）**：原局为格、动变为局。`legacy/core/physics/dynamic_engine.py` 提供：动态张量（引透权重翻倍、刑冲合化、地理阻尼 λ）；流形捕获（位移矢量、格局对撞态）；对撞预警（S 轴 + 流形不稳定性），返回 `source_pattern`/`target_pattern`/`collision_type` 供 RAG。判词须含「此年由 X 格转入 Y 局，防范 Z 类风险」。配置见 `legacy/config/dynamic_manifold.json`（干支刑冲合化、geo_damping、double_capture_ratio_threshold）。  
...
**第 048 号纠偏（古典提纲+透干）**：海选 A-01～A-10 须过「月令本气透干」硬约束（月令所藏透出天干方能论格），逻辑见 `legacy/core/classical_tougan.py`；`legacy/scripts/build_a01_full_index.py` 默认启用。禄刃 A-09/A-10 以十二长生（临官/帝旺）为纲。重刷后 A-01～A-10 总样本数不得超过 518k 的 20%，自检脚本 `legacy/scripts/audit/audit_048_abundance_check.py`。详见 `audit/FDS_048_古典提纲透干纠偏.md`（相对本索引所在 `legacy/docs/`）。
...
**并网后合拢**：DuckDB 秒级审计 SQL 示例见 `audit/FDS_045_DuckDB_OLAP_Examples.md`（含 A-04 身强财弱坍缩查询）；2.0 全量物理基线 `python3 legacy/scripts/audit_v2_physics_baseline.py` → `legacy/audit_logs/v2_0_physics_baseline.json`。
...
V4.x 及历史归档稿已从仓库移除；演进说明以本节「版本历史」及当前 **sop/**、**audit/** 正文为准。
...
### 测试与回归
- **测试说明与回归清单**: 见 `legacy/tests/README.md`
- 覆盖：FDS 推理引擎、TMM、全息控制器 FDS 分支（A-01/A-02/A-03）、AI 判词/格局解读、全量索引、V5.0 检查清单
- 运行: `pip install -r requirements-test.txt` 后执行 `python -m pytest legacy/tests/ -v`（仓库根目录）
