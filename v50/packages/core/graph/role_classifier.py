...
def _roles_for_node(
    *,
    ...
    if _is_engine_node(node, graph):
        roles.append((NodeRoleType.ENGINE_NODE, 0.68, ["role.structural_engine_candidate", "role.active_element_source"]))
    if _is_buffer_node(node, graph):
        roles.append((NodeRoleType.BUFFER_NODE, 0.72, ["role.storage_or_damp_buffer"]))
    if _is_single_failure_node(node, graph):
        roles.append((NodeRoleType.SINGLE_FAILURE_NODE, 0.82, ["role.explicit_structural_dependency", "role.removal_sensitive_candidate"]))
    return roles


def _is_engine_node(node: MingliGraphNode, graph: MingliGraph) -> bool:
    if node.position == "month_branch":
        return True
    if node.attributes.get("output_converter"):
        return True
...
def _is_single_failure_node(node: MingliGraphNode, graph: MingliGraph) -> bool:
    if node.attributes.get("triple_combination_bridge"):
        return True
    if node.attributes.get("output_converter"):
        return True
    return node.position == "day_branch" and any(
        edge.edge_type.value == "roots"
