...
def main():
    ...
    # Annual: 丙午 (Bing Fire - owl for Earth DM)
    test_annual = ('丙', '午')
    
    for chart in synth.generate_all_bazi():
        total_scanned += 1
        # Inject standard Luck/Annual
        test_chart = chart + [('壬', '申'), test_annual]
        
        res = scout._deep_audit(test_chart, "SSSC_AMPLIFIER")
        if res:
