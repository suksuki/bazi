...
def test_physics_meta_final_verdict_probe_relaxed_v1307():
    from app.services.analysis_service import _physics_meta_final_verdict_probe_relaxed

    assert _physics_meta_final_verdict_probe_relaxed({"meta": {}}) is False
    assert _physics_meta_final_verdict_probe_relaxed({"meta": {"v1299_convergence_skip": True}}) is True
    assert _physics_meta_final_verdict_probe_relaxed({"meta": {"v1301_two_stage_convergence_v1": True}}) is True


def test_tensor_bridge_fills_verified_fact_lines_from_conflict_matrix() -> None:
    from app.services.analysis_service import _tensor_bridge_fill_verified_fact_lines

    md = {"conflict_matrix": {"points": [{"kind": "harm", "detail": "寅巳穿害"}, {"detail": "子午冲"}]}}
    pt: dict = {"meta": {"semantic_label_bundle_v1": {"verified_fact_lines": []}}}
    _tensor_bridge_fill_verified_fact_lines(pt, md)
    vf = pt["meta"]["semantic_label_bundle_v1"]["verified_fact_lines"]
    assert len(vf) >= 3
    assert any("寅巳穿害" in str(x) for x in vf)
    assert any("检测到物理冲突" in str(x) for x in vf)


def test_merge_metadata_hot_reload_preserves_incremental_dict() -> None:
    from app.services.analysis_service import _merge_metadata_hot_reload

    base = {
        "incremental_context_v14": {"logical_patches": [{"a": 1}], "x": 1},
        "foo": "bar",
    }
    carry = {"incremental_context_v14": {"logical_patches": [{"b": 2}]}, "foo": "baz"}
    out = _merge_metadata_hot_reload(base, carry)
    assert out["foo"] == "baz"
    patches = (out.get("incremental_context_v14") or {}).get("logical_patches") or []
    assert patches == [{"b": 2}]
    assert (out.get("incremental_context_v14") or {}).get("x") == 1


@pytest.mark.asyncio
async def test_hot_reload_physics_context_merges_and_tags_meta(monkeypatch: pytest.MonkeyPatch) -> None:
    from app.api.contracts import FourPillars, HotReloadPhysicsRequest
    from app.services import analysis_service as m

    async def fake_clash(_body):
        return {
            "metadata": {"k": 1},
            "physics_tensor": {"meta": {}},
            "llm_prompt": [],
    ...
    monkeypatch.setattr(m, "analyze_clash_flow", fake_clash)
    pillars = FourPillars(
    ...
    body = HotReloadPhysicsRequest(
        pillars=pillars,
        metadata_carryover={"incremental_context_v14": {"logical_patches": [{"t": 1}]}},
    )
    out = await m.hot_reload_physics_context(body)
    assert out.get("hot_reload", {}).get("protocol") == "hot_reload_physics.v14"
    md = out.get("metadata") or {}
    assert (md.get("incremental_context_v14") or {}).get("logical_patches")
    pt = out.get("physics_tensor") or {}
    assert (pt.get("meta") or {}).get("hot_reload_protocol") == "v14.00"


def test_tensor_bridge_skips_when_verified_fact_lines_sufficient() -> None:
    from app.services.analysis_service import _tensor_bridge_fill_verified_fact_lines

    md = {"conflict_matrix": {"points": [{"detail": "X"}]}}
    pt = {
        "meta": {
            "semantic_label_bundle_v1": {
                "verified_fact_lines": ["一", "二", "三"],
            }
        }
    }
    _tensor_bridge_fill_verified_fact_lines(pt, md)
    assert pt["meta"]["semantic_label_bundle_v1"]["verified_fact_lines"] == ["一", "二", "三"]


def test_build_antecedent_verdict_v0_5_from_fragments() -> None:
    from app.services.analysis_service import _build_antecedent_verdict_v0_5_from_fragments

    md = {
        "narrative_fragments_v14": [
            {"narrative_fragment": "检测到寅巳穿害，先稳住主轴。"},
            {"narrative_fragment": "财星过热，需限制外放。"},
        ]
    }
    s = _build_antecedent_verdict_v0_5_from_fragments(md)
    assert s.startswith("【V0.5 初始断言】")
    assert "寅巳穿害" in s
