...
def test_weak_mode_never_forces_physics_fallback() -> None:
    """断语优先：不再因证据锚缺失而触发 physics 兜底。"""
    bad_anchor = {
        "assertions": [
            {"assertion_id": "a1", "text": "x", "evidence_refs": ["not.a.real.anchor"]},
        ]
    }
    assert weak_mode_requires_physics_fallback(bad_anchor, high_reasoning=False) is False
    assert weak_mode_requires_physics_fallback(bad_anchor, high_reasoning=True) is False
