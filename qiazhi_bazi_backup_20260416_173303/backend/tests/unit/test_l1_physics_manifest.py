...
def test_l1_physics_manifest_loads():
    ...
    assert "base.physics.op_branch_banhe" in ids
    assert "base.physics.op_stem_fusion_transformed" in ids
    assert "base.physics.op_sub_branch_interaction" not in ids
    assert "base.physics.op_lab_timing_weights" in ids
