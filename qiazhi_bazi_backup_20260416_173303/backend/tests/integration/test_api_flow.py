...
def test_analyze_clash_returns_atomic_points_without_llm(monkeypatch, tmp_path: Path):
    ...
    llm_prompt = str(result.get("llm_prompt") or "")
    assert ("是否需要深入分析这个局部" in llm_prompt) or ("FACT_NODE" in llm_prompt)
...
def test_analyze_seed_end_to_end(monkeypatch, tmp_path: Path):
    ...
    result = asyncio.run(
        router_module.analyze_seed(
            StandardSeedRequest(date="1977-05-08", time="18:00", calendar="solar", gender="male"),
