...
def _inbox_top_two_score_spread(meta: Dict[str, Any]) -> float:
    ...
    for x in rows:
        if isinstance(x, dict):
            try:
                scores.append(float(x.get("score") or 0.0))
...
def _life_death_polarity_tension(metadata: Dict[str, Any]) -> bool:
    """结构终裁包中的极性反转 + 红区/高压，视为「生死域」级张力（V13.01）。"""
    for key in ("final_structure_final_decision_v0", "structure_final_decision_v0"):
        blk = metadata.get(key)
        if not isinstance(blk, dict):
            continue
        rep = blk.get("plugin_conflict_report")
        if not isinstance(rep, dict):
            continue
        if not bool(rep.get("has_polarity_reversal")):
            continue
        zone = str(rep.get("zone") or "").strip().upper()
        tension = float(rep.get("tension_level") or 0.0)
        if zone == "RED" or tension > 0.75:
            return True
    return False
...
def evaluate_active_probing(
    *,
    ...
    p_interrupt = (
        persistence.get("interrupt_request")
        if isinstance(persistence.get("interrupt_request"), dict)
        else {}
    )
    p_interrupt_state = str((p_interrupt or {}).get("state") or "").strip().lower()
    resume_history = (
        persistence.get("resume_feedback_history")
        if isinstance(persistence.get("resume_feedback_history"), list)
        else []
    )
    resume_acked = p_interrupt_state in {"resumed", "resolved", "acknowledged"} and len(resume_history) > 0
    ...
    sgjg = str(l1_flags.get("sgjg_severity") or "").strip().upper()
    spread = _inbox_top_two_score_spread(meta)
    life_death = _life_death_polarity_tension(md)
    critish = sgjg == "CRITICAL" or bool(l1_flags.get("l1_inbox_signal_bypass"))
    # V13.01：CRITICAL / has_critical_marker 单点不再强阻断；仅 FATAL，或「生死域极性 + inbox 分差极小 + CRITICAL/bypass」才挂起
    severe_logic_conflict = (sgjg == "FATAL") or (life_death and spread < 0.08 and critish)
    if severe_logic_conflict and resume_acked:
        return ActiveProbingDecision(
            should_probe=True,
            block_mode=False,
            reason_code="M3_L1_LOGIC_CONFLICT_ACKED",
            probe_plan=[
                "已记录冲突确认反馈：本轮解除强阻断，继续执行局部重算与审计。",
                "如后续出现新的关键冲突，再次触发挂起确认。",
            ],
        )
