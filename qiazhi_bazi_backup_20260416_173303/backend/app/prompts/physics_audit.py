...
_DIAGNOSIS_ALIAS_RULE = (
    "diagnosis 字段严禁出现任何参数键名（如 CF_FLOATING_DECAY / THROUGH_STEM_BOOST / CONFLICT_PENALTY_GAMMA / "
    "A_PROTRUSION / OFFICER_RESTRAINT_ALPHA / POWER_DISTRIBUTION_GAMMA）。"
    "若需引用参数，只能使用中文别名：[虚浮衰减系数]、[透干增益系数]、[冲突惩罚强度]、[透出系数]、[官杀约束强度]、[功率分配指数]。"
)
_INTERNAL_SYSTEM_INSTRUCTION = (
    "[INTERNAL_SYSTEM_INSTRUCTION]"
    "可使用 sql_patch 与 logic_proposal 相关术语，但仅允许出现在对应 JSON 字段中。"
    "在 diagnosis 与 causal_reasoning 中严禁出现 sql/sql_patch/param_key/update/database/table 等工程术语。"
    "若需表达调参含义，使用命理语义或中文别名，不得暴露参数键名。"
)
    ...
    *,
    ...
    if t == "compact":
        system_text = (
            ...
            "偏差与补丁意图通过结构化字段承载；diagnosis/causal_reasoning 仅允许命理自然语言。"
