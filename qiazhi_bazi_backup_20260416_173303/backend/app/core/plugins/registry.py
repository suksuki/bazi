...
def plugin_authority_level(plugin_id: str) -> int:
    """
    V13.03：插件权力等级 1–5（DecisionHub / Inbox）。
    高等级对低等级结论具有一票否决语义：仅同最高等级插件参与后续 LLM 仲裁与 Inbox 打架。
    """
    ...
    if pid == "base.chronos":
        return 4
    if pid.startswith("classical.pattern"):
        return 4
    if pid in ("classical.blind_school.v1", "classical.wangshuai.v1"):
        return 3
    if pid.startswith("classical."):
        return 2
    if pid.startswith("modern."):
        return 2
    return 1
