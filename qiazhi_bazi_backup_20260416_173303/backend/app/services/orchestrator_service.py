...
from app.schemas.bazi_metadata import BaziMetadata, BrainHubPersistence, FlowState, PersistenceLayer
...
def _coerce_flow_state_slug(raw: Any) -> str:
    """统一 flow_state 为 schema slug（Resume 请求体里常见 Enum / str 混用）。"""
    if raw is None:
        return ""
    val: Any = getattr(raw, "value", raw)
    if isinstance(val, FlowState):
        val = val.value
...
class OrchestratorService:
    ...
    def run_internal_loop(
        *,
        ...
        try:
            apply_decision_inbox_pipeline(
                physics_tensor=physics_tensor,
                plugin_outputs=plugin_outputs,
                registry=ps.registry,
                metadata=metadata_obj.model_dump(mode="python"),
            )
        except Exception:
        ...
        next_flow_state = FlowState.PROBE_WAITING if interrupt_request else FlowState.UNKNOWN
        # V12.96：禁止「PROBE_WAITING + 空中断载荷」导致前端 Fallback 空壳
        if next_flow_state == FlowState.PROBE_WAITING and (
            not isinstance(interrupt_request, dict)
            or not (
                str(interrupt_request.get("probe_query") or "").strip()
                or str(interrupt_request.get("reason_code") or "").strip()
                or str(interrupt_request.get("interrupt_id") or "").strip()
            )
        ):
            ap_reason = str(active_probing.get("reason_code") or "").strip() or "PROBE_PENDING"
            ap_probe = str(active_probing.get("probe_query") or active_probing.get("why_probe") or "").strip()
            interrupt_request = {
                "state": "pending",
                "reason_code": ap_reason,
                "probe_query": ap_probe or f"请确认本轮逻辑断点（{ap_reason}）。",
                "interrupt_id": f"probe-guard-{session_id or 0}",
                "source": "orchestrator_probe_wait_guard_v12_96",
            }
            next_flow_state = FlowState.PROBE_WAITING
        persistence_obj = (
            metadata_obj.persistence_layer
            if isinstance(metadata_obj.persistence_layer, PersistenceLayer)
            else PersistenceLayer()
        )
        if interrupt_request:
            persistence_interrupt = dict(interrupt_request)
        else:
            persistence_interrupt = dict(persistence_obj.interrupt_request or {})
            st_old = str(persistence_interrupt.get("state") or "").strip().lower()
            if st_old in {"resumed", "acknowledged", "resolved", "expired"}:
                persistence_interrupt = {}
            elif persistence_interrupt and st_old == "pending":
                persistence_interrupt["state"] = "resolved"
                persistence_interrupt["resolved_at"] = (
                    datetime.now(timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
                )
        persistence_obj = persistence_obj.model_copy(update={"interrupt_request": persistence_interrupt})
        metadata_obj = metadata_obj.model_copy(
            update={
                "flow_state": next_flow_state,
                "persistence_layer": persistence_obj,
            }
        )
        if isinstance(physics_tensor.get("meta"), dict):
    ...
    def resume_calculation(
        *,
        ...
        interrupted_node_id = "UNKNOWN_NODE"
        ...
        with session_scope() as db:
            ...
            interrupt_src = (
                persistence.interrupt_request
                if isinstance(persistence.interrupt_request, dict)
                else (input_meta.get("interrupt_request") if isinstance(input_meta.get("interrupt_request"), dict) else {})
            )
            interrupt = dict(interrupt_src or {})
            meta_in = dict(metadata or {}) if isinstance(metadata, dict) else {}
            if not interrupt:
                pl_cli = meta_in.get("persistence_layer") if isinstance(meta_in.get("persistence_layer"), dict) else {}
                ir_cli = pl_cli.get("interrupt_request") if isinstance(pl_cli.get("interrupt_request"), dict) else {}
                if isinstance(ir_cli, dict) and ir_cli:
                    interrupt = dict(ir_cli)
            if not interrupt:
                fs_cli = _coerce_flow_state_slug(meta_in.get("flow_state") or input_meta.get("flow_state"))
                if fs_cli and fs_cli != "probe_waiting":
                    idempotent_resume = True
                    _LOG.info(
                        "resume idempotent: empty DB interrupt, flow_state=%s session_id=%s",
                        fs_cli,
                        session_id,
                    )
                    md_payload = dict(input_meta)
                    if meta_in:
                        for k, v in meta_in.items():
                            if v is not None:
                                md_payload[k] = v
                    ack_token = f"resume:idempotent-flow-{session_id}"
                    interrupted_node_id = "UNKNOWN_NODE"
                    consultation.input_meta = input_meta
                    db.flush()
                elif fs_cli == "probe_waiting":
                    # persistence 已无 pending 中断，但元数据仍标 PROBE（常见于 Resume 清空后未及时再 analyze）
                    idempotent_resume = True
                    _LOG.info(
                        "resume idempotent: empty interrupt with probe_waiting (stale) session_id=%s",
                        session_id,
                    )
                    md_payload = dict(input_meta)
                    if meta_in:
                        for k, v in meta_in.items():
                            if v is not None:
                                md_payload[k] = v
                    md_payload["flow_state"] = "unknown"
                    pl_norm = dict(md_payload.get("persistence_layer") or {})
                    pl_norm["interrupt_request"] = {}
                    md_payload["persistence_layer"] = pl_norm
                    ack_token = f"resume:idempotent-probe-stale-{session_id}"
                    interrupted_node_id = "UNKNOWN_NODE"
                    consultation.input_meta = input_meta
                    db.flush()
                else:
                    raise PermissionError("resume requires interrupt_request")
            elif not idempotent_resume:
                istate = str(interrupt.get("state") or "pending").strip().lower()
                if istate != "pending":
                    if istate not in {"resumed", "resolved", "acknowledged", "expired"}:
                        raise PermissionError("resume requires a pending interrupt_request")
                    idempotent_resume = True
                    _LOG.info(
                        "resume idempotent: interrupt already terminal state=%s session_id=%s",
                        istate,
                        session_id,
                    )
                    ack_token = str(interrupt.get("resume_ack_token") or "").strip() or f"resume:idempotent-{session_id}"
                    ...
                    if isinstance(metadata, dict) and metadata:
                        for k in ("pillars", "conflict_matrix", "temporal_context", "flow_state"):
                            if k in metadata:
                                md_payload[k] = metadata[k]
                    consultation.input_meta = input_meta
                    db.flush()
            if not idempotent_resume and interrupt:
                ...
                # V13.01：Resume 成功后不再把旧 interrupt_id 写回 persistence，避免前端缓存态反复复弹同一弹窗
                persistence = persistence.model_copy(update={"interrupt_request": {}})
        ...
        return {
            ...
            "idempotent": bool(idempotent_resume),
            "is_idempotent_success": bool(idempotent_resume),
