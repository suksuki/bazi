...
/** V13.05：与后端 ``PHASE_*`` 对齐的展示文案（message 缺省时回退） */
const THOUGHT_PHASE_LABELS: Record<string, string> = {
  PHASE_PHYSICS: "解析天干地支能量场...",
  PHASE_PLUGINS: "多维命理插件博弈中...",
  PHASE_ARBITER: "AI 正在批量裁决逻辑分歧...",
  PHASE_SYNTHESIS: "汇聚意志，生成终审报告...",
};
...
export interface BoardCommandPanelProps {
  ...
  /** 自动链路阶段（第 N 步） */
  autoRunStepIndex?: number;
  autoRunStepLabel?: string;
  ...
  /** V13.05：analyze-seed NDJSON 当前阶段 */
  analyzeSeedThoughtPhase?: AnalyzeSeedThoughtPhase;
  /** V13.05：全量掐指进行中，主按钮 Loading 转圈 */
  showSeedCalcSpinner?: boolean;
  /** 自动链路执行中：主按钮保持锁定，避免重复触发 */
  autoRunLocked?: boolean;
}
...
export function BoardCommandPanel({
  ...
  /** V12.93：Decision Inbox 为建议项，默认折叠以降低「冲突失控」压迫感 */
  const [logicReferenceOpen, setLogicReferenceOpen] = React.useState(false);
  React.useEffect(() => {
    if (probeWaiting) setLogicReferenceOpen(false);
  }, [probeWaiting]);
  ...
  return (
    ...
    >
      ...
      <UnifiedActionBar
        ...
        disabled={(actionMode === "FULL" && !draftSeed) || isFinalized || autoRunLocked}
        ...
        busySpinner={Boolean(showSeedCalcSpinner)}
      />
      <div className="mt-2 rounded-xl border border-zinc-800/80 bg-zinc-950/40 px-3 py-2 text-[11px]">
        {autoRunLocked ? (
          <p className="font-medium text-violet-200/95">
            自动推演中：第 {Math.max(1, autoRunStepIndex)} 步（{autoRunStepLabel || "准备中"}）
          </p>
        ) : (
          <p className="text-zinc-300/90">
            {analyzeSeedThoughtPhase?.message?.trim()
              ? analyzeSeedThoughtPhase.message
              : analyzeSeedThoughtPhase?.phase
                ? THOUGHT_PHASE_LABELS[analyzeSeedThoughtPhase.phase] || "—"
                : t("暂无活跃子任务（等待全量测算或流式通道）。")}
          </p>
        )}
      </div>
      <details
        className="relative rounded-xl transition-[box-shadow,opacity] duration-300"
        open={logicReferenceOpen}
        onToggle={(e) => setLogicReferenceOpen((e.target as HTMLDetailsElement).open)}
      >
        <summary className="cursor-pointer select-none list-none px-3 py-2 text-left text-xs font-medium text-zinc-300 outline-none marker:content-none [&::-webkit-details-marker]:hidden">
          <span className="text-cyan-200/95">{t("逻辑分歧参考")}</span>
          <span className="ml-2 font-mono text-[11px] text-zinc-500">（{cards.length} 项 · 建议项，非强阻断）</span>
        </summary>
      ...
      <DecisionInbox
        ...
        metadata={metadata || {}}
        physicsTensor={physicsTensorSnapshot}
        ...
        hideTopologyAndPatternPanels
        minimalAssertionOnly
      />
        </div>
      </details>
      {pulseReplay?.overlay ? (
