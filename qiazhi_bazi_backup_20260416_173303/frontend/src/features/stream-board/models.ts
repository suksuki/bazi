...
/** V13.05：analyze-seed NDJSON 推送的当前计算阶段（与后端 ``PHASE_*`` 对齐） */
export type AnalyzeSeedThoughtPhase = { phase: string; message: string } | null;
...
export type StreamBoardViewModel = {
  ...
  /** 与 FinalVerdictView（LiveVerdictDisplay）联动：十神条是否叠化注塑前虚线参考 */
  ...
  cards: InboxCard[];
  /** 排除占位卡 `fallback-deep-scan` 后的 Inbox 条数；主栏分流与 L1 Certified 与之一致 */
  ...
  /** V12.96：Resume 防抖窗口内为 true */
  resumeSubmitBusy?: boolean;
  resumeFromInterrupt: (
    feedback: {
  ...
  /** V13.05：全量 analyze-seed 流式阶段，供主栏文案与「思维进度」折叠区 */
