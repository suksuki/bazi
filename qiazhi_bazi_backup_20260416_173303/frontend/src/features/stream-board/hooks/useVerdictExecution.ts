...
const SILENT_FINAL_VERDICT_FLOW_CONFLICT = "SilentFinalVerdictFlowConflict";
...
function normalizeMetadataForFinalVerdict(md: BaziMetadata | null): BaziMetadata | null {
  if (!md) return md;
  const flow = String(md.flow_state || "").trim().toLowerCase();
  if (flow !== "probe_waiting") return md;
  const pl = (md.persistence_layer || {}) as { interrupt_request?: Record<string, unknown> };
  const ir = pl.interrupt_request;
  const hasPendingProbe =
    !!ir &&
    typeof ir === "object" &&
    !Array.isArray(ir) &&
    String((ir as Record<string, unknown>).probe_query || "").trim().length > 0;
  // 假性锁：flow_state=probe_waiting 但无可执行 probe_query，直接归一为 ready 以放行终判
  if (!hasPendingProbe) {
    return { ...md, flow_state: "ready" as unknown as BaziMetadata["flow_state"] };
  }
  return md;
}
...
export type VerdictExecutionDeps = {
  ...
  /** V12.96：终判 409（FLOW_STATE 冲突）时静默收敛状态，不抛错、不弹窗 */
  resolveFinalVerdict409Silently?: () => void;
  ...
  while (true) {
    ...
    for (;;) {
      ...
      if (ev.type === "token" && typeof ev.text === "string") {
        ...
        if (Number(ev.status_code || 0) === 409 && code === "FINAL_VERDICT_FLOW_STATE_CONFLICT") {
          const err = new Error("SILENT_FINAL_VERDICT_FLOW_STATE_CONFLICT");
          err.name = SILENT_FINAL_VERDICT_FLOW_CONFLICT;
          throw err;
        }
        const mapped = mapFinalVerdictError(Number(ev.status_code || 0), code);
        if (mapped && (Number(ev.status_code || 0) === 409 || code === "FINAL_VERDICT_FLOW_STATE_CONFLICT")) {
    ...
    async (
      ...
      try {
        try {
          ...
          try {
          const metadataForVerdict = normalizeMetadataForFinalVerdict(opts?.metadataForRequest ?? d.metadata);
          const jsonBody = JSON.stringify(
            buildFinalVerdictRequestBody({
              metadata: d.metadata,
              metadataForRequest: metadataForVerdict,
          ...
          if (tryStream) {
            ...
            if (sRes.ok && sRes.body) {
              try {
                ...
                if (error instanceof Error && error.name === SILENT_FINAL_VERDICT_FLOW_CONFLICT) {
                  d.resolveFinalVerdict409Silently?.();
          ...
          if (!response.ok) {
            ...
            const detailCode = String((detail as { code?: string }).code || "");
            if (Number(response.status || 0) === 409 && detailCode === "FINAL_VERDICT_FLOW_STATE_CONFLICT") {
              d.resolveFinalVerdict409Silently?.();
              return buildFallbackVerdict(conflicts);
            }
            const mapped = mapFinalVerdictError(Number(response.status || 0), detailCode);
            if (mapped && (Number(response.status || 0) === 409 || detailCode === "FINAL_VERDICT_FLOW_STATE_CONFLICT")) {
              emitVerdictFlowConflictNavigationPulse();
            }
