...
/** 解析 FastAPI 409 的 detail（string | object | array），便于幂等匹配。 */
function flattenOrchestratorResume409Detail(raw: string): string {
  const parsed = parseHttp409Detail(raw);
  return parsed.text;
}

/** 解析 FastAPI 409 的 detail，提取结构化 code（供幂等 / 静默分支）。 */
function parseHttp409Detail(raw: string): { text: string; code?: string } {
  const s = String(raw || "").trim();
  if (!s) return { text: "" };
  try {
    const p = JSON.parse(s) as { detail?: unknown };
    const d = p.detail;
    if (typeof d === "string") return { text: d };
    if (Array.isArray(d))
      return { text: d.map((x) => (typeof x === "string" ? x : JSON.stringify(x))).join(" ") };
    if (d && typeof d === "object") {
      const o = d as Record<string, unknown>;
      const code = typeof o.code === "string" ? o.code : undefined;
      if (typeof o.message === "string") return { text: o.message, code };
      if (typeof o.user_message === "string") return { text: o.user_message, code };
      return { text: JSON.stringify(o), code };
    }
    return { text: s };
  } catch {
    return { text: s };
...
export function useStreamBoardController(): StreamBoardViewModel {
  ...
  const [analyzeSeedThoughtPhase, setAnalyzeSeedThoughtPhase] = useState<AnalyzeSeedThoughtPhase>(null);
  ...
  /** V12.96：Resume 提交后物理防抖窗口（与 inFlight 同步释放） */
  const [resumeSubmitBusy, setResumeSubmitBusy] = useState(false);
  ...
  const resumeFromInterruptInFlightRef = useRef(false);
  ...
  const resumeFromInterrupt = useCallback(
    async (
      ...
      const mdForResume = options?.metadataOverride ?? metadata;
      if (!mdForResume) throw new Error("metadata missing");
      if (resumeFromInterruptInFlightRef.current) return;
      resumeFromInterruptInFlightRef.current = true;
      setResumeSubmitBusy(true);
      const compactFeedback = Object.fromEntries(
        Object.entries(feedback).map(([k, v]) => [k, typeof v === "string" ? v.substring(0, 300) : v]),
      ) as typeof feedback;
      const releaseResumeGate = () => {
        window.setTimeout(() => {
          resumeFromInterruptInFlightRef.current = false;
          setResumeSubmitBusy(false);
        }, 2000);
      };
      let response: Response;
      try {
        response = await fetch(`${API_BASE}/api/v1/orchestrator/resume`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            session_id: consultationId,
            user_feedback: compactFeedback,
            metadata: mdForResume,
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          const parsed = parseHttp409Detail(text);
          if (parsed.code === "FINAL_VERDICT_FLOW_STATE_CONFLICT") {
            setInterruptRequest(null);
            setMetadata((prev) => (prev ? { ...prev, flow_state: "ready" } : prev));
            mergeSnapshot({
              metadata: { ...(mdForResume as unknown as Record<string, unknown>), flow_state: "ready" },
            });
            releaseResumeGate();
            return;
          }
          ...
          const benign =
            /resume\s*requires|interrupt_request|pending\s*interrupt|待处理.*中断|中断请求|幂等|idempotent/i.test(
              detail,
            ) || detail.length === 0;
          if (benign) {
            appendSystemAuditLog(
              "Resume 与后端状态已对齐（409）：中断已处理或无需重复提交；已收起追问层并解除 PROBE_WAITING。",
            );
            setInterruptRequest(null);
            setMetadata((prev) => (prev ? { ...prev, flow_state: "unknown" } : prev));
            mergeSnapshot({
              metadata: { ...(mdForResume as unknown as Record<string, unknown>), flow_state: "unknown" },
            });
            releaseResumeGate();
            return;
          }
      ...
      const mdProbe = (nextMeta || mdForResume) as { flow_state?: string; persistence_layer?: { interrupt_request?: Record<string, unknown> } } | null;
      const fs = String(mdProbe?.flow_state || "").trim().toLowerCase();
      const persistedIr = mdProbe?.persistence_layer?.interrupt_request;
      const safeInterrupt =
        nextInterrupt && Object.keys(nextInterrupt).length > 0
          ? nextInterrupt
          : fs === "probe_waiting" && persistedIr && typeof persistedIr === "object" && Object.keys(persistedIr).length > 0
            ? persistedIr
            : null;
      setInterruptRequest(safeInterrupt && Object.keys(safeInterrupt).length > 0 ? safeInterrupt : null);
      mergeSnapshot({
        metadata: (nextMeta || mdForResume) as unknown as Record<string, unknown>,
        physics_tensor: (nextTensor || {}) as Record<string, unknown>,
      });
      const idem = Boolean(data.is_idempotent_success || data.idempotent);
      appendSystemAuditLog(
        idem
          ? "🧩 Resume 幂等成功：中断已在终态，已同步最新编排状态。"
          : "🧩 Resume 事务已提交：已从中断点执行局部重算。",
      );
      releaseResumeGate();
    },
    [
  ...
  verdictDepsRef.current = {
    ...
    setMetadata,
    setInterruptRequest,
    resolveFinalVerdict409Silently: () => {
      setInterruptRequest(null);
      const m = metadataRef.current;
      if (m) {
        const next = { ...m, flow_state: "ready" } as BaziMetadata;
        setMetadata(next);
        mergeSnapshot({ metadata: next as unknown as Record<string, unknown> });
      }
    },
  };

  /** V13.07：analyze-seed（含 NDJSON complete）完成后，useSeedAnalysis 据 physics_tensor.meta 收敛标记解除 PROBE 假性锁。 */
