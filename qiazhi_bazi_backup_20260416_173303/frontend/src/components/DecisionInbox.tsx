"use client";
/** V13：实验室裁决舱壳组件；终判流式断言子区已统一走 `FinalVerdictView`（再导出 `LiveVerdictDisplay`）。 */
...
import { FinalVerdictView } from "@/features/stream-board/components/FinalVerdictView";
...
import { PurpleVerdictCard } from "@/features/verdict/PurpleVerdictCard";
...
type Props = {
  ...
  /** physics_tensor：终判语义区在 SYSTEM_FALLBACK 时穿插 VF 脱水行 */
  ...
  /** 终判流式/合并完成后递增，驱动 FinalVerdictView 强制刷新 */
  ...
  /** 主页面瘦身：隐藏拓扑与格局引力，改由视觉仪表盘承载 */
  hideTopologyAndPatternPanels?: boolean;
  /** 主页面极简模式：仅保留断言主文与证据快照 */
  minimalAssertionOnly?: boolean;
};
...
export function DecisionInbox({
  ...
  hideTopologyAndPatternPanels = false,
  minimalAssertionOnly = false,
}: Props) {
  ...
  const [narratorRefreshTick, setNarratorRefreshTick] = useState(0);
  ...
  function applySelection(next: Record<string, boolean>) {
    ...
    const changedId = Object.keys(next).find((k) => Boolean(next[k]) !== Boolean(selectedIds[k]));
    const changedTo = changedId ? Boolean(next[changedId]) : null;
    if (typeof window !== "undefined" && changedTo !== null) {
      const actionId = `act-${Date.now()}-${String(changedId || "").slice(0, 16)}`;
      const optimisticText = changedTo ? "正在根据意志裁断：已采纳该冲突路径..." : "正在根据意志裁断：已撤回该冲突路径...";
      window.dispatchEvent(
        new CustomEvent("qiazhi:will-impact", {
          detail: {
            verb: changedTo ? "ACK" : "IGNORE",
            card_id: changedId || "",
            action_id: actionId,
            render_text: optimisticText,
          },
        }),
      );
    }
  ...
  return (
    ...
    <section className="relative z-10 p-4">
      ...
      <section className="mt-4 rounded-xl border border-zinc-800 bg-zinc-950 p-2">
        ...
        {sectionOpen.summary ? (
      <div className="mt-2 min-w-0 max-w-full overflow-x-hidden rounded-xl border border-zinc-800 bg-zinc-950 p-3">
        ...
        <div className="mt-2 min-w-0 max-w-full space-y-1">
          <PurpleVerdictCard
            ...
            godOfUse={useful}
            godOfTaboo={obstacle}
            reactionKey={narratorRefreshTick}
          />
          <div className="sr-only">{climateSummary}</div>
          {false ? (
          <FinalVerdictView
          ...
          {!minimalAssertionOnly && (verdictChangeLog.physics_diff?.length || verdictChangeLog.consensus_diff?.length || verdictChangeLog.text_diff_hint) ? (
          ...
          {!minimalAssertionOnly && !hideTopologyAndPatternPanels ? (
            <>
              <div className="mt-2 rounded-md border border-zinc-700 bg-zinc-900 p-2 text-[11px]">
                <button
                  type="button"
          ...
          {!minimalAssertionOnly && typeof (structureFinalDecision as { primary_structure?: unknown }).primary_structure === "string" ? (
            <div className="mt-2 rounded-md border border-zinc-700 bg-zinc-900 p-2 text-[11px]">
              <button
                type="button"
