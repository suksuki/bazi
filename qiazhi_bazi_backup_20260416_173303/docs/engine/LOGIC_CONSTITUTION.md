...
- Includes:
  ...
  - Standard interactions: 刑/冲/克/害/合/破/穿/墓库.
  - Climate correction as medium-density adjustment.
...
- Rules:
  ...
  - Yong/Ji from "suppress excess, support weakness".
...
- Rules:
  - 宾主/体用 segmentation.
  - Work vectors over interaction edges.
  - Backfire when tool-path is reversed or broken.
...
- `structure_final_decision_v0` MUST output:
  - `balance_verdict`
  - `work_verdict`
- `primary_structure_humanized` remains naming output, not a replacement for dual verdict.
- L1 interaction delta extraction stays as next-step refactor item; current phase keeps compatibility while hardening dual-verdict outputs.

## 5) Governance

- Any future skill change touching L1/L2 boundary must update this file first.
- If two school verdicts conflict, both must be shown; do not overwrite one with the other.
- Risk bands are recommended over single-point cutoffs:
