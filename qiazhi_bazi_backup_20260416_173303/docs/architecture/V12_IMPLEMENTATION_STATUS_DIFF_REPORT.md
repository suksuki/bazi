...
## 里程碑 6：V12.92 收官热补丁（状态冲突智能引导 + M5 监控站）

- 当前行为：
  - `/v1/final-verdict` 已将 `PROBE_WAITING` 分流为 `409 FINAL_VERDICT_FLOW_STATE_CONFLICT`，与 `422 V12_SCHEMA_VIOLATION_ERROR` 明确区分。
  - `useVerdictExecution` 已按后端 `status/code` 映射中文语义，并在 409 冲突时触发 `qiazhi:verdict-flow-conflict` 导航脉冲。
  - `StreamBoardView` 新增「冲突智能引导」：自动滚动并高亮 `InterruptOverlay`，并提供“立即前往确认核心逻辑”快捷按钮。
  - `PROBE_WAITING` 下主动作条半锁定（文案：`⚠ 待逻辑确认（PROBE_WAITING）`），源头减少误触终判。
  - `InterruptOverlay` 新增前端兜底：即便后端漏发 `interrupt_request`，只要 `flow_state=probe_waiting` 也可渲染可点击确认入口。
  - `/v1/brain/m5-gold-stats` 已支持降级返回（`degraded=true`），DB/Schema 暂不可用时不再返回 500。
- 与目标差异：
  - 终判冲突后的“自动打开紫色脉冲回放详情”尚未默认开启，当前仅实现滚动与高亮引导。
- 剩余风险：
  - 线上若混跑旧前端静态资源，仍会出现“已锁定但无入口”错觉；需确保前端全实例完成滚动部署。
