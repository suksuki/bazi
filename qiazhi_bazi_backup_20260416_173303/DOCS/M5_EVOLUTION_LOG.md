...
## 2026-04-14 Hotfix Notes

- final-verdict 状态冲突分流：`409 FINAL_VERDICT_FLOW_STATE_CONFLICT`（不再与 422 混淆）。
- 前端已接入冲突智能引导：`qiazhi:verdict-flow-conflict` 导航脉冲 + 快速跳转按钮。
- `m5-gold-stats` 已支持降级返回（`degraded=true`），避免 DB 不可用时监控站 500 风暴。
