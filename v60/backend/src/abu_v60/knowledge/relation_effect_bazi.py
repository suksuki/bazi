...
RELATION_EFFECT_ADMISSION_POLICY_ID: Final = "v60.relation-effect-admission-boundary.v1"
RELATION_EFFECT_RULE_PROPOSAL_ID: Final = "v60.relation-effect-proposal.zi-wu-automatic-damage.v1"


@lru_cache(maxsize=1)
def bazi_relation_effect_admission_policy() -> BaziRelationEffectAdmissionPolicy:
...
@lru_cache(maxsize=1)
def bazi_zi_wu_automatic_damage_proposal() -> BaziRelationEffectRuleProposal:
    return BaziRelationEffectRuleProposal.issue(
        claim=("子午六冲成员命中后，自动判定午中同干来源受损，并据此判定该来源不可用。"),
        dimension_submissions=(
            ...
            RelationEffectProposalDimension(
                ...
                status="COMPETING",
                statement=(
                    "只扰动支关系、打开或暴露来源、损伤或移除来源仍是互相竞争且均未准入的解释。"
                ),
            ),
            RelationEffectProposalDimension(
