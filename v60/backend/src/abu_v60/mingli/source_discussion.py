...
SOURCE_DISCUSSION_REQUIRED_UNRESOLVED_DIMENSIONS = frozenset({"relation_effect", "usability"})
    ...
    def project(
        ...
        *,
        ...
        return MingliSourceDiscussionAbstentionReceipt.issue(
            ...
            carrier_refs=tuple(carrier.carrier_ref for carrier in prerequisite.carriers),
            carrier_count=prerequisite.carrier_count,
            ready_carrier_count=prerequisite.ready_carrier_count,
            blocking_requirement_ids=self._ordered_requirement_ids(blocking_statuses),
            non_triggered_requirement_ids=self._ordered_requirement_ids(non_triggered_statuses),
        )
    ...
    def _validated_inputs(
        *,
        ...
        if prerequisite.ready_carrier_count != 0 or any(
            ...
            raise ValueError("source_discussion_ready_carrier_not_supported_by_v001")
        validated_reading = MingliReadingEnvelope.model_validate(reading.model_dump(mode="python"))
        validated_prerequisite = MingliSourceUsabilityPrerequisiteEnvelope.model_validate(
        ...
        if validated_prerequisite.prerequisite_version != SOURCE_USABILITY_PREREQUISITE_VERSION:
            raise ValueError("source_discussion_prerequisite_version_not_supported")
        ...
        if source_review_profile.runtime_scope != SOURCE_REVIEW_RUNTIME_SCOPE:
            raise ValueError("source_discussion_source_review_runtime_scope_mismatch")
        if source_review_profile.professionally_reviewed:
            raise ValueError("source_discussion_professionally_reviewed_profile_not_supported")
        if not SOURCE_DISCUSSION_REQUIRED_UNRESOLVED_DIMENSIONS <= set(
            validated_reading.unresolved_dimensions
        ):
            raise ValueError("source_discussion_reading_unresolved_dimensions_missing")
    ...
    def _validate_lineage(
        *,
        ...
        if (
            ...
            or prerequisite.source_review_vector_ref != reading.source_review_vector_ref
            or prerequisite.source_review_vector_hash != reading.source_review_vector_hash
        ):
            raise ValueError("source_discussion_source_review_vector_lineage_mismatch")
