...
def test_lab_candidate_projection_is_read_only_and_owned_by_mingli() -> None:
    ...
    assert "v131_mingli_lab_experience_asset_parcel" in modules["media"].capabilities
    ...
    assert "append_only_agent_normalization_receipt" in (
        modules["mingli"].capabilities
    )
    ...
    assert "model_raw_normalized_trace_inspection" in (
        modules["unit-lab"].capabilities
    )
    assert "reviewer_scoped_synthetic_method_lab" in (
        modules["unit-lab"].capabilities
    )
    assert "v131_research_watercourt_overview" in modules["unit-lab"].capabilities
