#!/usr/bin/env python3
"""
FDS SOP V5.8：第三梯队（A-31～A-35）奇格 Phase 3 迁库
========================================================
在 Phase 1+2 海选通过后执行：提取 L1 匹配样本的 5D 张量，写入 DuckDB pattern_points（替换 TMM_SEED）。
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="V5.8 第三梯队 Phase 3: A-31～A-35 奇格迁库")
    p.add_argument("--data", type=Path, default=None)
    p.add_argument("--abundance", type=Path, default=None, help="默认 audit_logs/v58_batch_3_abundance.json")
    p.add_argument("--limit", type=int, default=1000, help="每格局最多迁入条数（默认 1000）")
    args = p.parse_args()
    ...
    abundance_path = args.abundance or (ROOT / "audit_logs" / "v58_batch_3_abundance.json")
    if not abundance_path.exists():
        print(f"❌ 未找到丰度报告: {abundance_path}，请先运行 run_v58_batch_3_scan.py")
        sys.exit(1)
