#!/usr/bin/env python3
"""
FDS SOP V6.1：第六梯队（A-46～A-60）Phase 3 迁库
=================================================
在 Batch 6 海选通过后执行，将 A-46～A-50 等 L1 匹配样本写入 DuckDB。
"""
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="V6.1 第六梯队 Phase 3 迁库")
    p.add_argument("--data", type=Path, default=None)
    p.add_argument("--abundance", type=Path, default=None, help="默认 audit_logs/v61_batch_6_abundance.json")
    p.add_argument("--limit", type=int, default=2000, help="每格局最多迁入条数（默认 2000）")
    args = p.parse_args()
    ...
    abundance_path = args.abundance or (ROOT / "audit_logs" / "v61_batch_6_abundance.json")
    if not abundance_path.exists():
        print(f"❌ 未找到丰度报告: {abundance_path}，请先运行 run_v61_batch_6_scan.py")
        sys.exit(1)
